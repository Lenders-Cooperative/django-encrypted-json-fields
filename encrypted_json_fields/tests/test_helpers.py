...
class GetDefaultCrypterTest(TestCase):
    ...
    def test_get_default_crypter_aes(self):
        """Test that the default crypter returns an AESCBCEncryption instance."""
        crypter = get_default_crypter(self.keys)
        self.assertIsInstance(crypter, AESCBCEncryption)
    ...
    def test_get_default_crypter_invalid_setting(self):
        ...
        self.assertIn(
            "Encryption method 'nonexistentencryption' is not a registered encryption class.", str(context.exception)
        )
